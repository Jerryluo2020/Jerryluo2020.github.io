...
async function read<T>(url: string, signal: AbortSignal, body?: unknown): Promise<T> {
  const r = await apiFetch(url, { signal, cache: 'no-store', ...(body ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : {}) });
...
export default function PatternMatcher({ snapshotDate, minSampleDate }: { snapshotDate?: string; minSampleDate?: string } = {}) {
  const [code, setCode] = useState(''), [start, setStart] = useState(''), [finish, setFinish] = useState(''), [date, setDate] = useState(snapshotDate || completeDate());
  ...
  return <section className="pattern-panel" aria-label="自选样本相似走势筛选">
    <div className="pattern-heading"><div><div className="eyebrow">PATTERN MATCH</div><h2>自选样本 · 相似走势</h2><p>{data ? `${sampleName}（${data.reference.stock.code}） · ${data.reference.start}—${data.reference.end}` : '选择一只A股及一段历史走势，寻找最接近的5只股票'}</p></div><span className="pattern-badge">{data?.reference.provider === 'tencent' ? '五因子 · 日线数据' : '六因子评分'}</span></div>
    <form className="pattern-controls" onSubmit={e => { e.preventDefault(); void scan({code:code.trim(),start,end:finish},date); }}>
      ...
      <label>样本开始日期<Input aria-label="样本开始日期" type="date" required value={start} max={finish || snapshotDate || completeDate()} min={minSampleDate || "1990-12-19"} onChange={e=>edit(setStart,e.target.value)} disabled={busy}/></label>
      <label>样本结束日期<Input aria-label="样本结束日期" type="date" required value={finish} max={snapshotDate || completeDate()} min={start || '1990-12-19'} onChange={e=>edit(setFinish,e.target.value)} disabled={busy}/></label>
      <label>筛选截止日<Input aria-label="筛选截止日" type="date" required value={date} max={snapshotDate || completeDate()} min={finish} onChange={e=>edit(setDate,e.target.value)} disabled={busy}/></label>
    ...
    {data && <>
      {data.reference.provider === 'tencent' && <div className="pattern-source-note">本次使用腾讯财经日线。成交额、换手率缺失，换手率不参与评分，其余五项权重按比例归一化。</div>}
      ...
      <details className="pattern-details"><summary>筛选口径与数据来源</summary><p>从新浪财经当前供应商A股股票池中，取截至 {data.date} 的最近 {sample.length} 个交易日，与样本逐日对齐。剔除样本股票自身、ST、退市标记、区间停牌及预热数据不足的股票。历史日期筛选使用当前股票池，存在存续偏差，不能作为严格历史回测。</p><p>价格使用首日收盘归一化对数路径；量能为当日成交量/此前5日均量；均线为收盘价相对MA5/10/20的偏离；MACD(12,26,9)按股价归一化；RSI使用14日Wilder平滑；换手率使用log(1+x)。各项距离为均方根差，固定尺度分别为0.10、0.8、0.05、0.02、25、1。综合分=100×exp(-加权距离)，至少60根日线预热。权重未经收益回测，不预示后续收益。</p><p><a href={data.reference.provider === 'tencent' ? `https://gu.qq.com/${sampleSymbol}` : `https://quote.eastmoney.com/${sampleSymbol}.html`} target="_blank" rel="noreferrer">{data.reference.source}</a> · 前复权价格会随后续公司行动修订；每天收盘后更新快照，实际日期以页面数据状态为准。{data.generatedAt && `本次计算：${new Date(data.generatedAt).toLocaleString('zh-CN',{timeZone:'Asia/Shanghai'})}（北京时间）。`}选择筛选截止日后点击筛选，将读取当前快照并在浏览器计算；关闭页面会停止进行中的扫描。</p></details>
    </>}
  </section>;
}
